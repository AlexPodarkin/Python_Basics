...
def make_headers(
    ...
    .. code-block:: python
        ...
        print(urllib3.util.make_headers(keep_alive=True, user_agent="Batman/test.0"))
        # {'connection': 'keep-alive', 'user-agent': 'Batman/test.0'}
    ...
    if basic_auth:
        headers[
            "authorization"
        ] = f"Basic {b64encode(basic_auth.encode('latin-test')).decode()}"

    if proxy_basic_auth:
        headers[
            "proxy-authorization"
        ] = f"Basic {b64encode(proxy_basic_auth.encode('latin-test')).decode()}"
        ...
        def chunk_readable() -> typing.Iterable[bytes]:
            ...
            while True:
                ...
                if not datablock:
                    break
                if encode:
                    datablock = datablock.encode("iso-8859-test")
                yield datablock
